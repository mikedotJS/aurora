...
const SYSTEM_PROMPT = [
  ...
  "Each workspace has an absolute $AURORA_PORT env var already resolved to this workspace's reserved",
  "port (managed-server-lifecycle) — append --port $AURORA_PORT directly, with NO arithmetic. Examples:",
  "  next dev  →  next dev --port $AURORA_PORT",
  "  vite      →  vite --port $AURORA_PORT",
  "  nx serve api   →  nx serve api --port $AURORA_PORT",
  "  nx serve web   →  nx serve web --port $AURORA_PORT",
  "  ng serve       →  ng serve --port $AURORA_PORT",
  "  rails server   →  rails server -p $AURORA_PORT",
  "In a multi-server repo, give each server its own task, each using $AURORA_PORT (the allocator",
  "reserves a whole range per workspace, not just one port).",
  "A server that already honours $PORT may use $PORT directly instead.",
  "Do NOT use a fixed port number, and do NOT use the legacy $((DEFAULT + AURORA_PORT_OFFSET)) form —",
  "always use the plain $AURORA_PORT variable.",
  "For `npm run`/`yarn run`/`pnpm run` script wrappers, pass flags to the underlying tool AFTER `--`",
  "(e.g. `npm run dev -- --port $AURORA_PORT`). Prefer the direct binary",
  ...
  '[{"name":"dev","desc":"run the dev server","split":false,"tasks":[{"dir":"","cmd":"npm run dev -- --port $AURORA_PORT"}]}]',
