# merge-request-search Specification

## Purpose
Let users quickly find a merge request in the MR sheet by typing a text search (over title, branch, author, and `!iid`) and by filtering to merge requests they authored ("mine"), with all filtering applied client-side against the cached list and driving the sheet's keyboard navigation.
